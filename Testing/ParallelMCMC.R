...
cluster <- makeCluster(no_cores)  #No. of cores to use (max 12, but don't use all)
...
for (iteration in -N0 : N){
  ...
  while (no_proposals < no_top_props){
    ...
    clusterExport(cluster, list('topology_proposals',
                                'ed.mig.birth', 'ed.mig.death',
                                'ed.mig.pair.birth', 'ed.mig.pair.death',
                                'ed.coal.split', 'ed.coal.merge',
                                'ed.block.recolour',
                                'ScaledLikelihoodC',
                                'ED_like',
                                'top.probs', 'U', 'ED', 'n_deme', 'node_indices',
                                'coal_rate', 'mig_mat', 'time_scale'))

    top_proposals <- parLapply(cluster, 1:(no_top_props - no_proposals), topology_proposals)
  ...
  clusterExport(cluster, list('topology_proposals',
                              'scaled_coal_rate_gibbs_update',
                              'scaled_mig_mat_gibbs_update',
                              'ScaledLikelihoodC',
                              'ED_like',
                              'cr_prior_shape', 'cr_prior_rate', 'coal_rate_prior',
                              'mm_prior_shape', 'mm_prior_rate', 'mig_mat_prior',
                              'ED', 'n_deme', 'node_indices',
                              'coal_rate', 'mig_mat', 'time_scale'))
  cont_updates <- parLapply(cluster, 1:2, continuous_proposals)
